...
public class BuscaPedido extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			calendar = Conversao.textoEmDataHoraLocal(request
		...
		try {
			...
			calendar1 = Conversao.textoEmDataHoraLocal(request
			...
			if (p != null) {
				...
				response.getWriter().write(
						"<input type=\"hidden\" id=\"id" + i + "\" "
								+ "value=\"" + p.getId() + "\">"
								+ "<input type=\"hidden\" id=\"valorTotal" + i
								+ "\" " + "value=\"" + p.getValor() + "\">"
								+ "<input type=\"hidden\" id=\"descricao" + i
								+ "\" " + "value=\"" + p.getDescricao() + "\">"
								+ "<input type=\"hidden\" id=\"fornecedor" + i
								+ "\" " + "value=\""
